...
public:
  ...
  Buffer()
      : readerIndex_(kForPrepend), writerIndex_(kForPrepend),
        capacity_(kForPrepend + kInitialSize), buffer_(new char[capacity_]) {
...
private:
  ...
  char *end() { return buffer_ + capacity_; }
  ...
  void makeSpace(size_t len) {
    ...
    if (writerIndex_ >= readerIndex_) {
      memcpy(newbuffer_ + kForPrepend, buffer_ + readerIndex_,
             (writerIndex_ - readerIndex_));
    } else {
      // writerIndex_在前的时候需要考虑 可读数据分为后 前两段的情况
      memcpy(newbuffer_ + kForPrepend, buffer_ + readerIndex_,
             capacity_ - readerIndex_);
      memcpy(newbuffer_ + kForPrepend + capacity_ - readerIndex_, buffer_,
             writerIndex_);
    }
